...
startSendingMsg::(NodeStateIM,Int,Int,Int,[ProcessId]) -> Process ()
...
broadCast::Int->NodeStateIM->Int->BroadCastingGroup->Process ()
broadCast sendFor nodeStateVecIM seed (BG senderId n recvs) =
       void $ spawn n $ $(mkClosure 'startSendingMsg) (nodeStateVecIM,senderId,sendFor,seed,recvs)

-- | Given a list of NodeIds makeBroadCastGroups returns a list
--   of BroadcastingGroups
makeBroadCastGroups::[(NodeId,NodeStateIM)]->Process [BroadCastingGroup]
